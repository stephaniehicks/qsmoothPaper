...
#### smoking data set
# Code to create the alveolarSmokingAffyData object can be found here: https://github.com/stephaniehicks/qsmoothPaper/tree/master/scripts/createDataObjects/create_alveolarSmokingAffyData.R
# The alveolarSmokingAffyData object was saved into an R data package
...
matdensity(log2pmMat, groupFactor = pd$Group, xlab = "log2 PM values", 
           ylab = " ", lwd = 2, ylim = c(0, 0.36), lty = c(1,2,3)[factor(pd$Group)])
legend('topright', lgroup, lwd = 2, col = 1:3, lty = c(1,2,3))
dev.off()
...
matdensity(log2pmMatQN, groupFactor = pd$Group, xlab = "log2 PM values", 
           ylab = " ", lwd = 2, ylim = c(0, 0.36), lty = c(1,2,3)[factor(pd$Group)])
legend('topright', levels(pd$Group), lwd = 2, col = 1:3, lty = c(1,2,3))
dev.off()
...
#### tcruzi data
# Code to create the Tcruzi ExpressionSet object can be found here: https://github.com/stephaniehicks/qsmoothPaper/tree/master/scripts/createDataObjects/create_trcuziData.R
load(file.path(fig_path, "data/tcruzi/tcruzi_eset.rda"))
...
# Code to create the brain and liver Affy object can be found here:
# https://github.com/stephaniehicks/qsmoothPaper/tree/master/scripts/createDataObjects/create_BrainandLiverData.md
...
matplot(x = output$x, output$densityMat, type = "l", 
        ...
        lty=c(1,2)[factor(pdSub$tissue)])
legend('topright', c("Brain", "Liver"), lwd = 2, 
       col = c(colGreen[6], colRed[6]), lty=c(1,2))
dev.off()
...
pdf(file.path(fig_path, "Figures/rat_density_liver_brain.pdf"),
    width = 8, height = 10)
par(mfrow=c(3,2),mar=c(2, 2, 1, .5), mgp=c(0.7, 0.1, 0), tck=0.02)
...
matdensity(lpcounts, groupFactor=pData(subdataRat)$organ, 
           ...
           lty = c(1,2)[factor(pData(subdataRat)$organ)])
...
rownames(lpcounts) <- rownames(exprs(subdataRat))
matdensity(lpcounts, groupFactor=pData(subdataRat)$organ, 
           ylim = c(0, 0.17), xlab="log2(count + 1)", ylab=" ", 
           lty = c(1,2)[factor(pData(subdataRat)$organ)])
...
# Figure 5: Density plots, boxplots and PCA plots using Flowsorted DNAm data (Beta values)
...
# create figure
pdf(file.path(fig_path, "Figures/FlowSorted_density_boxplot_pca_BetaValues.pdf"),
    width = 12, height = 12)

# raw 
par(mfrow=c(3,3))
matdensity(p0, groupFactor = pd0$CellType, xlab = "beta values", ylab = "density", 
           main = "raw (no normalization)", lty=c(1:6)[factor(pd0$CellType)])
legend('top', levels(factor(pd0$CellType)), col = 1:6, lwd = 3, lty=1:6)
...
plot(scores$X1, scores$X2, pch = c(15:18,3:4)[factor(pd0$CellType)], 
     col = c(1:6)[factor(pd0$CellType)], 
...
# qsmooth
betaFlowSortedBlood450kQsmooth <- qsmoothData(qsmooth(p0, groupFactor = factor(pd0$CellType)))
# load(file.path(fig_path,"data/betaFlowSortedBlood450kQsmooth.RData"))
matdensity(betaFlowSortedBlood450kQsmooth, groupFactor = pd0$CellType, 
           main = "qsmooth", xlab = "beta values", ylab = "density", 
           lty=c(1:6)[factor(pd0$CellType)])
legend('top', levels(factor(pd0$CellType)), col = 1:6, lwd = 3, lty=1:6)

matboxplot(betaFlowSortedBlood450kQsmooth, groupFactor = pd0$CellType, 
           main = "qsmooth", ylab = "beta values", names = pd0$CellType)

s.qs <- svd(betaFlowSortedBlood450kQsmooth)
