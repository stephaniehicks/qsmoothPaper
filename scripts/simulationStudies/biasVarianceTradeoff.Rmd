...
calculateMSERandom <- function(simDiffsObject, nGroups = 2)
{
    ...
    trueDiffObject <- simDiffsObject[,1,]
    
    simBias = sapply(1:nMethods, function(x){ rowMeans(simDiffsObject[,(x+1),] - trueDiffObject) })
    simMSE = sapply(1:nMethods, function(x){ 
                  rowMeans( (simDiffsObject[,(x+1),] - trueDiffObject)^2 ) })
    simVar <- colMeans(simMSE) - colMeans((simBias)^2)

    list( abs(colMeans(simBias)), # bias
          colMeans((simBias)^2),  # bias^2
          simVar,                 # variance
          colMeans(simMSE))       # MSE
}
...
for(ind in 1:length(pDiffMax)){
     pDiff <- rep(pDiffMax[ind], N)
     coeffs = cbind(c(rep(truthVal, floor(pDiffMax[ind]*nGenes)), 
                     rep(0, nGenes - floor(pDiffMax[ind]*nGenes))))
     ...
     simResults <- foreach(i = 1:N) %dopar% 
      {
        ...
        sFsCols <- runif(nG, 0.5, 1.5)
        dat0 <- sweep(simData$counts, 2, sFsCols, FUN = "*")
        ...
        # observed difference in means using raw data
        fit <- rowttests(log2(dat0+1), factor(group))
        simDiffs[,1] <- fit$dm
    
        # observed difference in means using quantile normalization
        dat0.qn <- normalize.quantiles(dat0)
        fit <- rowttests(log2(dat0.qn+1), factor(group))
        simDiffs[,2] <- fit$dm
    
        # observed difference in means using qsmooth normalization
    ...
    dat[[ind]] = data.frame(gather(as.data.frame(tmp)),
                 "method" = factor(rep(c("raw", "qn", "qsmooth"), dim(tmp)[2]),
                                   c("raw", "qn", "qsmooth")))
}

datAll <- cbind(do.call("rbind", dat), "pDiff" = rep(pDiffMax, each = dim(dat[[1]])[1]))
levels(datAll$method) <- c("Raw", "QN", "qsmooth")

pBias <- datAll %>% filter(key %in% c("Bias2")) %>% 
    ggplot(aes(x = method, y = sqrt(value), fill = method)) + geom_bar(stat = "identity") + 
...
pVar <- datAll %>% filter(key %in% c("Var")) %>% 
    ...
    facet_wrap(~pDiff, nrow=1) + ylab("Variance") + xlab(" ") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
    guides(fill=guide_legend(title="Method"))
pRMSE <- datAll %>% filter(key %in% c("MSE")) %>%
    ggplot(aes(x = method, y = sqrt(value), fill = method)) + geom_bar(stat = "identity") +
    facet_wrap(~pDiff, nrow=1) + ylab("Root Mean Squared Error") + xlab(" ") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    guides(fill=guide_legend(title="Method"))

p <- plot_grid(pBias, pVar, pRMSE, ncol=1, labels = LETTERS[1:4])
p

pdf(file.path(fig_path, "Figures/Bias_Var.pdf"), width = 10, height = 12)
print(p)
dev.off()
